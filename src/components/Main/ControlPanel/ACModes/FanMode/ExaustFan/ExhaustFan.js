...
import c from './ExhaustFan.module.css';

const ExhaustFan = (props) => {
	let toggleMode = () => {
		let confirmation = window.confirm("You are about to turn on the Exhaust Fan mode. Please confirm the action.");
		if(confirmation === true) {
				props.systemData.isExhaustFanOn ?props.updateElementStatus('exhaustFan', false) : props.updateElementStatus('exhaustFan', true);
		}
	}
	return (
		<button className={props.systemData.isExhaustFanOn && c.active} onClick={toggleMode}>
			<div className={c.exhaustFanImage}>
				<svg version="1.1" id="Слой_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
					...
					<style type="text/css">
						{`.st7{fill:${props.systemData.isExhaustFanOn ? '#63CEBE' : '#343755'}`}
					</style>
					<path className="st7" d="M505.1,307.9l-2.6-7.9l-48.7-12.2c0.3-4.4,0.4-8.9,0.4-13.3c0-56.8-24.4-111-66.9-148.7l-19.9,22.4
			...
			<h4 className={props.systemData.isExhaustFanOn && c.active}>Exhaust</h4>
		</button>
	)
}
