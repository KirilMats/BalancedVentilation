...
import c from './FanMode.module.css';
import ExhaustFan from "./ExaustFan/ExhaustFan";
import SupplyFan from "./SupplyFan/SupplyFan";

const FanMode = (props) => {
	let toggleMode = () => {
		// if(props.systemData.isExhaustFanOn === true || props.systemData.isSupplyFanOn === true) {
		// 	let confirmation = window.confirm('You are about to turn off the Fan Mode. Please confirm the action.');
		// 	if(confirmation) {
		// 		props.updateElementStatus('exhaustFan', false);
		// 		props.updateElementStatus('supplyFan', false);
		// 		props.updateElementStatus('exhaustFan', false);
		// 	}
		// }
		props.modeData.isFanBarShown ? props.toggleFanBar(false) : props.toggleFanBar(true);
	}
	return (
		<div className={c.fanModeWrap}>
			<button disabled={!props.modeData.isSystemOn} className={`${c.fanModeButton} ${props.systemData.isExhaustFanOn || props.systemData.isSupplyFanOn ? c.active : 0}`} onClick={toggleMode}>
				<div className={c.fanImage}>
					<svg version="1.1" id="Слой_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
						...
						<style type="text/css">
							{`.st0{fill:${props.systemData.isExhaustFanOn || props.systemData.isSupplyFanOn  ? '#63CEBE' : '#343755'}`}
						</style>
						<path className="st0" d="M505.1,307.9l-2.6-7.9l-48.7-12.2c0.3-4.4,0.4-8.9,0.4-13.3c0-56.8-24.4-111-66.9-148.7l-19.9,22.4
				...
				<h4 className={props.systemData.isExhaustFanOn || props.systemData.isSupplyFanOn ? c.active : 0}>Fan</h4>
			</button>
			<div className={`${c.fanModesWrap} ${props.modeData.isFanBarShown && c.active}`}>
				<ExhaustFan {...props} />
				<SupplyFan {...props} />
			</div>
		</div>
